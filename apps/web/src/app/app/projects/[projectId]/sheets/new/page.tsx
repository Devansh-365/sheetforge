...
export default function NewSheetPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { sheet } = await connectSheet(projectId, googleSheetId, tabName.trim());
  ...
  return (
    <div className="max-w-xl">
      ...
      <h1 className="text-[38px] font-bold leading-[57px] mb-8">Connect a sheet</h1>

      <form onSubmit={onSubmit} className="space-y-6">
        <div>
          <label htmlFor="url" className="block text-sm mb-2" style={{ color: '#b8b2b2' }}>
          ...
          <p style={{ color: '#7f7a7a' }} className="text-sm mt-1">
            The sheet must be shared with your Google account. The first row is used as column
            headers.
          </p>
        </div>

        <div>
          <label htmlFor="tab" className="block text-sm mb-2" style={{ color: '#b8b2b2' }}>
            Tab name
          </label>
          <input
            id="tab"
